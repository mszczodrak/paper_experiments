...
for line in f.readlines():
	...
	if not(mote_id in nodes.keys()):
		new_node = {}
		new_node['last_start_done'] = None
		new_node['last_stop_done'] = None
	...
	if dbg == DBGS_RADIO_START_DONE:
		nodes[mote_id]['last_start_done'] = timestamp

		if nodes[mote_id]['last_stop_done'] != None:
			diff = timestamp - nodes[mote_id]['last_stop_done']
			if diff < 1:
				print "wrong"
			nodes[mote_id]['start_done_t'].append( diff )		

	if dbg == DBGS_RADIO_STOP_DONE:
		nodes[mote_id]['last_stop_done'] = timestamp

		if nodes[mote_id]['last_start_done'] != None:
			diff = timestamp - nodes[mote_id]['last_start_done']
			if diff < 1:
				print "wrong"
			nodes[mote_id]['stop_done_t'].append( diff )		
...
for mote_id in nodes.keys():
	on = np.array(nodes[mote_id]['start_done_t'])
	off = np.array(nodes[mote_id]['stop_done_t'])
