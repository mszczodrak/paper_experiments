...
def take_snapshot(trigger_node, radio_tx_power, radio_index, radio_change):
	new_snapshot = {}
	new_snapshot['node_id'] = trigger_node
	new_snapshot['radio_tx_power'] = radio_tx_power
	new_snapshot['radio_index'] = radio_index
	new_snapshot['radio_change'] = radio_change
	nodes_copy = copy.deepcopy(nodes)

	number_of_neighbors = []
	number_of_good_neighbors = []
	...
	for node_id in nodes_copy.keys():
		...
		number_of_good_neighbors.append( nodes_copy[node_id]['status_update']['good_quality_neighbors'] )
	...
	number_of_good_neighbors_array = np.array( number_of_good_neighbors )
	...
	new_snapshot['avg_number_of_good_neighbors'] = number_of_good_neighbors_array.mean()
	new_snapshot['std_number_of_good_neighbors'] = number_of_good_neighbors_array.std()
	new_snapshot['max_number_of_good_neighbors'] = number_of_good_neighbors_array.max()
	new_snapshot['min_number_of_good_neighbors'] = number_of_good_neighbors_array.min()
...
def add_node(mote_id):
	...
	new_node['status_update'] = {
		'neighborhood_count': 0, 'good_quality_neighbors': 0, 'neighbors_in_need': 0
	}
	new_node['seqno'] = 0
...
for line in f.readlines():
	...
	if dbg == DBGS_STATUS_UPDATE:
		...
		status['good_quality_neighbors'] = good_quality_neighbors
	...
	if dbg == DBGS_CHANNEL_RESET:
		...
		if radio_tx_power > nodes[mote_id]['radio_tx_power']:
			...
			take_snapshot(mote_id, radio_tx_power, radio_index, 'up')
		else:
			nodes[mote_id]['radio_tx_power'] = radio_tx_power
			take_snapshot(mote_id, radio_tx_power, radio_index, 'down')
	...
	if dbg == DBGS_ADD_NODE:
		...
		nodes[mote_id]['neighborhood'][sender_src]['etx'] = 0
	...
	if dbg == DBGS_GOT_RECEIVE:
		...
		nodes[mote_id]['neighborhood'][sender_src]['etx'] = 100 * sender_rec / sender_sequence
