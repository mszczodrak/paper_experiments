...
max_radio_index = conf.radio_power_index[conf.radio_tx_powers_confs[-1]]
...
print "Max Size: %d" % (max_radio_index)
	...
	for node_id in G.nodes():
		if node_id not in nodes.keys():
			...
			if 'radio_tx_power' in nodes[node_id].keys():
				...
				radio_index = 0
			nodes_powers.append( int(radio_index) )

	nodes_colors = nodes_powers

	nodes = nx.draw_networkx_nodes(G, position, cmap = plt.get_cmap('gray'),
		node_color = nodes_colors, with_labels=False,
		node_size=100,
		vmin = 0, vmax = max_radio_index)

	edges = nx.draw_networkx_edges(G, position, alpha=0.4)

	cbar = plt.colorbar(nodes, ticks=conf.radio_tx_power_indexs)
	cbar.ax.set_xticklabels(conf.radio_tx_powers_sorted)
	cbar.set_label('dB')
