...
if len(sys.argv) != 7:
	print "usage %s <data_to_import> <radio_conf> <etx_threshold> <m> <n> <title>\n" % (sys.argv[0])
	exit()
...
for node_id in nodes.keys():
	...
	good_index = len(per_node_best_etx)
	all_index = len(per_node_all_etx)
...
plt.title("Final Power Distribution (M:%d N:%d ETX:%d)" % (m, n, etx_threshold), y=1.12)
