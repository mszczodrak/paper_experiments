...
if len(sys.argv) != 7:
	print "usage %s <data_to_import> <radio_conf> <rssi_threshold> <m> <n> <title>\n" % (sys.argv[0])
	exit()

data_module = sys.argv[1]
fig_title = sys.argv[6]
rssi_threshold = int(float(sys.argv[3]))
file_name = "_".join(fig_title.split())

m = int(float(sys.argv[4]))
...
for node_id in nodes.keys():
	...
	good_index = nodes[node_id]['status_update']['good_quality_neighbors']
	all_index = nodes[node_id]['status_update']['neighborhood_count']
	
	if good_index not in good_neighbors.keys():
		good_neighbors[good_index] = 0

	good_neighbors[good_index] = good_neighbors[good_index] + 1
...
the_x = max(good_neighbors.values() + all_neighbors.values()) + 1

x_data = np.arange(the_x)

good_neighbors_y = [0] * the_x
for k in sorted(good_neighbors.keys()):
	i = good_neighbors[k]
	good_neighbors_y[i] = k
...
plt.title("Final Power Distribution (M:%d N:%d RSSI:-%d)" % (m, n, rssi_threshold), y=1.12)
...
r2 = ax1.bar(n_ticks, good_neighbors_y, width, color='r')
#ax1.set_xticks(n_ticks)
#ax1.set_xlabel('Number of neighbors')
#ax1.set_ylabel('Number of motes')
